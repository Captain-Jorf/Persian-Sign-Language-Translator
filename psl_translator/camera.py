...
from .constants import DEFAULT_SEQUENCE_LENGTH, LEFT_HAND, RIGHT_HAND
from .dataset import SignSample, write_jsonl
from .features import make_frame_vector
...
def _require_camera_stack():
    ...
    except ImportError as exc:  # pragma: no cover - machine dependent
        raise RuntimeError(
            "OpenCV/MediaPipe are not installed. Run: pip install -r requirements.txt"
        ) from exc
    return cv2, mp
...
def collect_samples(
    ...
    """Collect one webcam sample as JSONL landmarks."""
    cv2, mp = _require_camera_stack()
    ...
    try:
        while time() < deadline:
            ...
            cv2.putText(image, f"Recording: {label}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
def run_realtime_translation(
    ...
    try:
        while True:
            ...
            cv2.putText(image, current_text, (20, 45), cv2.FONT_HERSHEY_SIMPLEX, 1, (50, 220, 50), 2)
            cv2.imshow("Persian Sign Language Translator", image)
            if cv2.waitKey(1) & 0xFF == ord("q"):
                break
    finally:
        hands.close()
